...
def test_put_state_endpoint_with_payload(test_client):
    """
        Checks for a 200 status code when a PUT request is send over '/api/state with a payload
    """
    response = test_client.put('/api/state',data={'state':"INIT"})
    assert response.status_code == 200
...
def test_get_run_log_endpoint(test_client):
    ...
    assert response.status_code == 200
    assert b'INIT' in response.data or b'RUNNING' in response.data or\
